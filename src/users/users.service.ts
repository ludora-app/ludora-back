...
export class UsersService {
	...
	async create(
		...
		const formattedBirthdate = createUserDto.birthdate
	...
	async findAll(
		...
		const connectedUserSports = connectedUser.userSportPreferences.map(
			(p) => p.sport
		);
		const sportsToScore = isFilteringSports
			? filterSports
			: connectedUserSports;
		...
		if (connectedUserSportPrefs.length > 0) {
			const levelConditions = connectedUserSportPrefs.map(
				(p) =>
					Prisma.sql`(usp_lvl.sport::text = ${p.sport} AND usp_lvl.level = ${p.level})`
			);
    ...
    `;
		...
		const uidToIndex = new Map(
			itemsToFetch.map((item, idx) => [item.uid, idx])
		);
	...
	async findOneByStripeAccountId(
	...
	async updatePassword(
		...
		if (existingUser.provider !== Provider.LUDORA) {
			throw new BadRequestException(
				"Only LUDORA users can update their password"
			);
		}
